...
from app.storage.rag_documents import get_rag_document_repository
...
RAG_REWRITE_CACHE_TTL_SECONDS = 600
RAG_RETRIEVAL_CACHE_TTL_SECONDS = 1800
...
def get_current_knowledge_base_version(collection_name: str | None = None) -> str:
    repository = get_rag_document_repository()
    normalized_collection = collection_name or get_settings().rag_collection_name
    return str(repository.get_knowledge_base_version(normalized_collection))
...
def build_shared_cache_key(namespace: str, *, model_version: str, payload: Any) -> str:
    return build_cache_key(
        ...
        CacheKeyContext(
            ...
            workspace_id="shared",
            knowledge_base_version=get_current_knowledge_base_version(),
...
def cache_ttl_seconds(namespace: str) -> int | None:
    normalized_namespace = namespace.strip()
    if normalized_namespace == "rag:rewrite":
        return RAG_REWRITE_CACHE_TTL_SECONDS
    if normalized_namespace == "rag:retrieval":
        return RAG_RETRIEVAL_CACHE_TTL_SECONDS
    if normalized_namespace == "rag:answer":
        return RAG_ANSWER_CACHE_TTL_SECONDS
    return None
