...
export function MarkdownContent({ value }: MarkdownContentProps) {
  return (
    <div className="min-w-0 break-words text-[15px] leading-8 text-ink">
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        components={{
          a: ({ href, children }) => <SafeAnchor href={href}>{children}</SafeAnchor>,
          blockquote: ({ children }) => (
            <blockquote className="my-4 border-l-2 border-tide/30 pl-4 text-ink/70">{children}</blockquote>
          ),
          code: ({ className, children, ...props }: any) => {
            ...
            if (inline || !isBlock) {
              return (
                <code className="rounded bg-paper px-1.5 py-0.5 font-mono text-[0.92em] text-ink">
                  {children}
                </code>
              );
            }
          ...
          img: ({ alt, src }) => (
            <img
              alt={alt ?? ""}
              src={src ?? ""}
              className="my-4 max-w-full rounded-2xl border border-ink/10"
            />
          ),
          ...
          table: ({ children }) => (
            <div className="my-4 overflow-x-auto rounded-2xl border border-ink/10">
              <table className="min-w-full border-collapse text-left text-sm">{children}</table>
            </div>
          ),
          tbody: ({ children }) => <tbody className="divide-y divide-ink/10">{children}</tbody>,
          td: ({ children }) => <td className="border-r border-ink/10 px-3 py-2 align-top last:border-r-0">{children}</td>,
          th: ({ children }) => (
            <th className="border-r border-ink/10 bg-paper px-3 py-2 font-semibold text-ink last:border-r-0">
              {children}
            </th>
          ),
