    ...
    const paths = useMemo(() => {
        ...
        for (let index = 0; index < points.length - 1; index += 1) {
            ...
            if (current.row === next.row) {
                ...
                const endX = next.x;
                lines.push(
                    `M ${startX} ${current.y} V ${elbowY} H ${midX} V ${next.y - 22} H ${endX} V ${next.y}`
                );
            }
        }
        ...
        >
            ...
            <div className="relative space-y-5">
                <div className="flex items-center justify-between gap-4">
                    <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.4em] text-cyan-200/80">RAG Control
                            Plane</p>
                        <h3 className="mt-2 text-2xl font-black text-white">可视化入库链路与问答链路</h3>
                    </div>
                    <div
                        className="rounded-full border border-fuchsia-400/25 bg-fuchsia-400/10 px-3 py-1 text-xs font-medium text-fuchsia-100">
                        Excalidraw-inspired
                    </div>
                </div>
                <div ref={containerRef} className="relative">
                    <svg aria-hidden="true"
                        ...
                        {paths.map((path, index) => (
                            <path key={`${index}-${path}`} d={path} fill="none" stroke="url(#rag-line)"
                                  strokeWidth="2"
                                  strokeLinecap="round"/>
                        ))}
                    </svg>
                  ...
                  <span
                      className={`rounded-full border px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] ${kindStyles[step.kind]}`}>
                    {step.kind}
                  </span>
                                    <span
